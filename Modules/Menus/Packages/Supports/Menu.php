...
namespace Modules\Menus\Packages\Supports;
...
class Menu
{
    ...
    /**
     * Menu constructor.
     * @param MenusInterface $menuRepository
     * @param HtmlBuilder $html
     * @param MenusNodeInterface $menuNodeRepository
     * @param CacheManager $cache
     * @param Repository $config
     */
    public function __construct(
        MenusInterface $menuRepository,
        HtmlBuilder $html,
        MenusNodeInterface $menuNodeRepository,
        CacheManager $cache,
        Repository $config
    ) {
        $this->config = $config;
        ...
        $this->cache = new Cache($cache, MenusRepository::class);
        $this->data = collect([]);
    }
...
//                dd($hasChild);
    ...
    public function addMenuLocation(string $location, string $description): self
    {
        ...
        $this->config->set('menus.locations', $locations);
    ...
    {
        return $this->config->get('menus.locations', []);
    }
    ...
    {
        return $this->menuRepository->allBy(['status' => DboardStatus::PUBLISHED], ['menuNodes', 'locations']);
    }
    ...
    public function generateMenu(array $args = [])
    {
        ...
        $data['status'] = Arr::get($args, 'status', []);
        ...
        return view('menus::partials.default', $data)->render();
    }
    ...
    public function registerMenuOptions(string $model, string $name)
    {
        ...
        echo view('menus::menu-options', compact('options', 'name'));
    }
    ...
    public function generateSelect(array $args = [])
    {
        ...
        if (!Arr::has($args, 'items')) {
            ...
            if (Arr::get($args, 'active', true)) {
                $items = $items->where('status', DboardStatus::PUBLISHED);
            }
        ...
        return view('menus::partials.select', compact('items', 'model', 'options'))->render();
    }
