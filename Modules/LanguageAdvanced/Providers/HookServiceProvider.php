...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        add_filter(BASE_FILTER_BEFORE_GET_ADMIN_LIST_ITEM, [$this, 'checkItemLanguageBeforeAdminGetListItem'], 1134, 2);
...
//                Assets::addStylesDirectly('vendor/kamruldashboard/slug/css/slug.css');

                $prefix = SlugHelper::getPrefix(get_class($object));

                return view('languageadvanced::slug', compact('object', 'prefix'))->render();
            }
        ...
        add_filter('stored_meta_box_key', function (string $key, $object) {
            ...
            if (is_module_active('LanguageAdvanced') && $locale != Language::getDefaultLocaleCode() && in_array($key, $translatableColumns)) {
                $key = $locale . '_' . $key;
            }
...
//add_new_line_Interface_and_Repository_to_hook
    ...
    public function languageMetaField()
    {
        ...
        return view('languageadvanced::language-box', compact(
            'args',
            'languages',
            'currentLanguage',
            'route'
    ...
    public function addLanguageColumn($data, $model)
    {
        if ($model && LanguageAdvancedManager::isSupported($model)) {
            ...
            return $data->addColumn('language', function ($item) use ($model, $route) {
                ...
                return view('languageadvanced::language-column', compact('item', 'route', 'languages'))
                    ->render();
            });
        }
...
//        dd($array_route);
     ...
     * @return mixed
     */
    public function checkItemLanguageBeforeAdminGetListItem($data, $model)
    {
        if (!LanguageAdvancedManager::isSupported($model)) {
            return $data;
        }
...
//        return $data;
        ...
        $data = $data->where($translationTable . '.lang_code', $currentAdminLocale)->with([
            'translations' => function ($query) use ($model, $currentAdminLocale, $translationTable) {
                $query->where($translationTable . '.lang_code', $currentAdminLocale);
            },
        ])->join($translationTable, $translationTable . '.' . $table . '_id', $table . '.id');;
        return $data;
    }
