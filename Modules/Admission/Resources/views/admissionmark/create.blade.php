...
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('admissionmark.createadmissionmark.store') }}"  enctype="multipart/form-data">
@endif
@csrf
                ...
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            @php
                                if(isset($record)){
                                    $slug = get_slug_table_data(null, \Modules\Admission\Http\Models\AdmissionMark::class,$record->id, \Modules\Admission\Http\Models\AdmissionMark::class);
                                }
                            @endphp
...
{{--                            </div>--}}
        ...
        <div class=" col-md-3">
            ...
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Admission\Http\Models\AdmissionMark::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
