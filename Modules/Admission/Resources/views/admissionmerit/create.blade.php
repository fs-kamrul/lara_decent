...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'admissionmerit');
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\Admission\Http\Models\AdmissionMerit::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('admissionmerit.createadmissionmerit.store') }}"  enctype="multipart/form-data">
@endif
@csrf
        @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\Admission\Http\Models\AdmissionMerit::class) @endphp

    <div class="row">
        <div class=" col-md-9">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@lang('admission::lang.admissionmerit_create')</h4>
                </div>
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            @php
                                if(isset($record)){
                                    $slug = get_slug_table_data(null, \Modules\Admission\Http\Models\AdmissionMerit::class,$record->id, \Modules\Admission\Http\Models\AdmissionMerit::class);
                                }
                            @endphp
...
{{--                            </div>--}}
        ...
        <div class=" col-md-3">
            ...
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Admission\Http\Models\AdmissionMerit::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
