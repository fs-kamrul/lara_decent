...
class AdminPackageTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * AdminPackageTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param AdminPackageInterface $adminpackageRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, AdminPackageInterface $adminpackageRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $adminpackageRepository;

        if (!Auth::user()->can(['adminpackage_edit', 'adminpackage_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('adminpackage_edit')) {
                    ...
                    $name = Html::link(route('adminpackage.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'adminboard/adminpackage';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('adminpackage.edit', 'adminpackage.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('adminpackage.create'), 'adminpackage_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('adminpackage.deletes'), 'adminpackage_destroy', parent::bulkActions());
    }
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'status' => [
                'title' => trans('table::lang.status'),
                'type' => 'customSelect',
                'choices' => AdminPackageStatusEnum::labels(),
                'validate' => 'required',
            ],
            'created_at' => [
                'title' => trans('table::lang.created_at'),
