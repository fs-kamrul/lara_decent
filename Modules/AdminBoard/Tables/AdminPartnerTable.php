...
class AdminPartnerTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * AdminPartnerTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param AdminPartnerInterface $adminpartnerRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, AdminPartnerInterface $adminpartnerRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $adminpartnerRepository;

        if (!Auth::user()->can(['adminpartner_edit', 'adminpartner_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('adminpartner_edit')) {
                    ...
                    $name = Html::link(route('adminpartner.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'adminboard/adminpartner';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('adminpartner.edit', 'adminpartner.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('adminpartner.create'), 'adminpartner_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('adminpartner.deletes'), 'adminpartner_destroy', parent::bulkActions());
    }
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'status' => [
                'title' => trans('table::lang.status'),
                'type' => 'customSelect',
                'choices' => AdminPartnerStatusEnum::labels(),
                'validate' => 'required',
            ],
            'created_at' => [
                'title' => trans('table::lang.created_at'),
