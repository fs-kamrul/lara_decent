        ...
        Dropzone.options.dropzone =
            {
                ...
                init:function() {
                    ...
                    $.ajax({
                        url: '{{ route('admin.dropzone.getimages',  ["id" => $all_images, "model" => 'Modules\AdminBoard\Http\Models\AdminTestimonial']) }}',
                        type: 'GET',
                        dataType: 'json',
...
@section('content')
    ...
    @if(isset($record))
        @php
            $button = 'update';
            $language = getLanguageUrlPost($record->id , 'admintestimonial');
        @endphp
        <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
            <input type="hidden" name="model" value="{{ \Modules\AdminBoard\Http\Models\AdminTestimonial::class }}">
            <input type="hidden" name="language" value="{{ $language['code'] }}">
            @else
                @php
                    $button = 'save';
                @endphp
                <form method="POST" action="{{ route('admintestimonial.createadmintestimonial.store') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf
                    @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\AdminBoard\Http\Models\AdminTestimonial::class) @endphp

                    <div class="row">
                        <div class=" col-md-9">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">@lang('adminboard::lang.admintestimonial_create')</h4>
                                </div>
                                <div class="card-body">
                                    <div class="basic-form">
                                        <div class="form-row">
                                            ...
                                            @php
                                                if(isset($record)){
                                                    $slug = get_slug_table_data(null, \Modules\AdminBoard\Http\Models\AdminTestimonial::class,$record->id, \Modules\AdminBoard\Http\Models\AdminTestimonial::class);
                                                }
                                            @endphp
                                            ...
                                            {!! Form::textField('designation', isset($record) ? $record->designation : null, 'adminboard', '6', ['placeholder'=> __('adminboard::lang.designation')]) !!}
                                            {!! Form::textField('phone', isset($record) ? $record->phone : null, 'adminboard', '6', ['placeholder'=> __('adminboard::lang.phone')]) !!}
                                            {!! Form::textField('email', isset($record) ? $record->email : null, 'adminboard', '6', ['placeholder'=> __('adminboard::lang.email')]) !!}


{{--                                            <div class="form-group col-md-12" id="short_description">--}}
{{--                                                <label>@lang('adminboard::lang.short_description_or_seo_description')</label>--}}
{{--                                                --}}{{--                                <input type="text" id="short_description" name="short_description" value="@isset($record){{$record->short_description}}@else{{ old('short_description') }}@endisset" class="form-control" placeholder="@lang('adminboard::lang.short_description_or_seo_description')">--}}
{{--                                                <textarea class="form-control" rows="4" placeholder="@lang('adminboard::lang.short_description_or_seo_description')"--}}
...
{{--                                            </div>--}}
                                            <div class="col-xl-12 col-xxl-12 list-shortcode-items" id="content_set">
                                                <label><strong>@lang('adminboard::lang.content')</strong></label>
                                                <div class="card">
                                                    <div class="card-body">
                                                        <div class="btn-group" role="group">
                                                            <button class="btn btn-primary show-hide-editor-btn" type="button" data-result="content">
                                                                {{ trans('adminboard::lang.show_hide_editor') }}
                                                            </button>
                                                        </div>
                                                        <div class="btn-group" role="group">
                                                            <div class="basic-dropdown editor-action-item">
                                                                <div class="dropdown">
                                                                    <button type="button" class="btn btn-primary dropdown-toggle add_shortcode_btn_trigger" data-result="content" data-toggle="dropdown" data-bs-toggle="dropdown">
                                                                        <i class="fa fa-code"></i> {{ trans('adminboard::lang.short_code') }}
                                                                    </button>
...
{{--                                            </div>--}}
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', \Modules\AdminBoard\Http\Models\AdminTestimonial::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', new \Modules\AdminBoard\Http\Models\AdminTestimonial ) @endphp
                            @endisset
                        </div>
                        <div class=" col-md-3">
                            @isset($record)
                                        ...
                                        <div class="form-row">
                                            @isset($record)
                                                @if($language['code_edit'])
                                                    {!! status_admin_enum($record->status,12, '', \Modules\AdminBoard\Enums\AdminTestimonialStatusEnum::class) !!}
                                                @endif @else {!! status_admin_enum(1,12, '' , \Modules\AdminBoard\Enums\AdminTestimonialStatusEnum::class) !!}
                                            @endisset
                                        </div>
                                    </div>
                                </div>
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\AdminBoard\Http\Models\AdminTestimonial::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'top', new \Modules\AdminBoard\Http\Models\AdminTestimonial ) @endphp
                            @endisset
                        </div>
                    </div>
                </form>
