...
class Theme implements ThemeContract
{
     ...
     * @var string
     */
    public static $namespace = 'themes';
...
//        self::uses($this->getThemeName())->layout(setting('layout', 'other_page'));
    ...
    {
        $themeDir = $this->getConfig('themeDir') == null ? 'themes' : $this->getConfig('themeDir');
    ...
    public function getThemeNamespace(string $path = ''): string
    {
        ...
        if(static::$namespace == ''){
            $namespace = strtolower($this->getThemeName());
        }else{
            $namespace = strtolower(static::$namespace . '.' . $this->getThemeName());
        }
    ...
    public function getThemeName()
    {
        ...
        return Arr::first(scanFolder(theme_path()));
    }
    ...
    public function getPublicThemeName()
    {
        ...
        $publicThemeName = $this->config->get('theme.public_theme_name');
...
//            'Activated-License' => !empty(setting('licensed_to')) ? 'Yes' : 'No',
    ...
    {
        return File::requireOnce(module_path('theme','Routes/public.php'));
    }
    ...
    {
        $this->bodyAttributes = array_merge($this->bodyAttributes, $bodyAttributes);
    ...
    {
        return isset($this->bodyAttributes[$attribute]) ? $this->bodyAttributes[$attribute] : null;
    }
