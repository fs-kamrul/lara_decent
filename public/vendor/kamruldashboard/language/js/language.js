class LanguageManagement {
    ...
    bindEventToElement() {
            ...
            $.ajax({
                ...
                success: data => {
                    if (data.error) {
                        ...
                        star.replaceWith('<a data-section="' + languages_set + '?lang_id=' + star.data('id') + '" class="set-language-default tip" data-bs-original-title="Choose ' + star.data('name') + ' as default language">' + star.closest('td').html() + '</a>');
                        _self.find('i').unwrap();
                        $('.tooltip').remove();
                        kamruldashboard.showSuccess(data.message);
                    }
        ...
        languageTable.on('click', '.edit-language-button', event => {
            ...
            $.ajax({
                url: languages_get + '?lang_id=' + _self.data('id'),
                type: 'GET',
                success: data => {
                    if (data.error) {
                        kamruldashboard.showError(data.message);
    ...
    static createOrUpdateLanguage(id, name, locale, code, flag, order, isRTL, edit) {
        let url = languages_store;
        if (edit) {
            url = languages_edit + '?lang_code=' + code;
        }
